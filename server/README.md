# Backend (server) - API RESTful
Este módulo contiene la API RESTful construida con Express, Mongoose y TypeScript. Su principal responsabilidad es gestionar los datos del proyecto y ejecutar la lógica de negocio para la generación de listas inteligentes.
## 1. Configuración de Dependencias
Este proyecto utiliza ts-node-dev para el desarrollo, permitiendo el reinicio automático del servidor al detectar cambios en los archivos .ts.
Scripts Principales:
### Scripts Principales
| Comando | Descripción |
| :--- | :--- |
| `npm install` | Instala todas las dependencias del backend. |
| `npm run dev` | Inicia el servidor en modo desarrollo con reinicio automático (usa ts-node-dev). |
| `npm start` | Inicia el servidor en producción. |
### Variables de entorno
| Variable | Descripción  | Ejemplo de Valor |
| :--- | :--- | :--- |
| `PORT` | Puerto en el que se ejecuta el servidor Express. | 5000 |
| `Mongo URI` | Cadena de conexión a la base de datos MongoDB. | mongodb://localhost:27017/promartdb |
| `CLIENT_URL` | URL del Frontend, necesaria para la configuración de CORS. | http://localhost:5173 |

## 2. Estructura de Modulos
El proyecto sigue una estructura MVC (Model-View-Controller) adaptada a la arquitectura de servicios.
| Carpeta | Responsabilidad  |
| :--- | :--- |
| `config` | Configuración de la base de datos (Mongoose). |
| `controller` | Maneja la lógica de la petición HTTP y la respuesta. |
| `models` | Definición de Schemas y Modelos de Mongoose. |
| `routes` | Define los endpoints de la API. |
| `services` | Contiene la lógica de negocio compleja y la interacción con Mongoose. La lógica inteligente reside aquí. |
| `utils` | Herramientas auxiliares (ej: script de carga inicial). |

## 3. Base de datos principal (script completo)
